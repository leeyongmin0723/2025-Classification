...
![label_istribution](hospital.png)
...
※ 세종시는 크롤링 가능한 병원이 없어 병원 수 0으로 표시됨
(병원이 없다는 의미가 아님)
...
- 전체 리뷰 문장의 평균 길이는 약 41자이며 중앙값은 32자로 나타났다.
- 문장 길이는 10~50자 구간에 가장 많이 분포하며, 비교적 상세하게 경험을 서술하는 리뷰가 많은 특징을 보였다.
- 3자 이하의 의미 없는 단문(예: “굿”, “좋음”)은 분석 품질을 위해 제거하였다.
![label_istribution](sentence_length_grouped.png)
...
- 데이터 분할은 Stratified Split을 사용하여 라벨 비율을 유지하였으며, 이는 모델이 특정 라벨에 편향되지 않도록 하기 위한 조치이다.
---
