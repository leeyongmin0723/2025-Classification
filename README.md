...
## 1. 프로젝트 개요 및 배경
병원을 선택할 때 사람들은 보통  
**친절했는지, 설명이 충분했는지, 대기 시간은 어땠는지, 치료 효과는 있었는지**  
이런 핵심 정보만 빠르게 알고 싶어 했다.

하지만 실제 리뷰들은 길고 감정 표현이 많아  
원하는 정보를 한눈에 파악하기 어려웠다.

이 문제를 해결하기 위해 본 프로젝트에서는  
**병원 리뷰를 8개 카테고리로 자동 분류하는 KoELECTRA 기반 모델을 개발하기로 했다.**

이 프로젝트는  
**크롤링 → 전처리 → 제로샷 라벨링 → 라벨 검증 → 모델 학습 → 평가 → 토픽 분석**  
까지 전체 파이프라인을 직접 구축하며 진행된 프로젝트였다.
---
